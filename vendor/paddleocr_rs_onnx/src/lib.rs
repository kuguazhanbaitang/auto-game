...
fn configure_session_builder(
    ...
    match device {
        // [local patch] CPU 与未启用 feature 的 EP 一律回退 CPU。
        // 各硬件 EP 分支按对应 cargo feature 编译期启用，避免无条件引用
        // ort::ep::*（被 ort 的 feature gate）导致默认 feature 下编译失败。
        _ => Ok(builder),
...
impl OcrEngine {
    ...
    pub fn new_with_device(
        ...
        // [local patch] 动态维度（-1/0）回退到 PP-OCRv4 rec 标准高度 48：
        // 上游用 `-1.max(1) = 1` 把动态高度算成 1，导致 rec 输入被压成 1px 高、识别全空
        let rec_height = rec_shape
            .get(2)
            .copied()
            .filter(|dim| *dim > 0)
            .unwrap_or(48) as u32;
        ...
        info!(
            "[ocr] rec model input shape: {rec_shape:?} → rec_height={rec_height}, rec_width={rec_width:?}"
        );
